...
class Writer:
    ...
    #: Дорого ли обходится один файл.
    #:
    #: Замерено на главе в сорок абзацев: `.txt`, `.md`, `.fb2` — меньше
    #: миллисекунды, `.docx` — сорок. Из них половина уходит на разбор
    #: пустого шаблона Word (библиотека парсит его заново на каждый
    #: файл), половина — на упаковку zip. Отсюда пятьсот глав в Word за
    #: двадцать секунд при полусекунде на любой другой формат.
    #:
    #: Помеченные так работы раскладываются по ядрам: `ops.base.spread`
    #: смотрит именно на этот признак.
    heavy: bool = False
