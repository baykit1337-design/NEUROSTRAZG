...
class Chapter:
    ...
    source: str = ""
    #: Заголовок вынут из самого текста, а не взят из имени файла или из
    #: свойств документа.
    #:
    #: Разница видна только при записи. Название из имени файла в тексте
    #: не стояло — напиши его обратно, и оно появится в главе новой
    #: строкой, которой там не было. Работы, которые правят текст на
    #: месте (речь через тире), спрашивают именно это; тем, кто собирает
    #: главу заново (скачивание, разбиение), заголовок нужен всегда, и
    #: для них умолчание неважно.
    heading_from_text: bool = False
