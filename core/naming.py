...
#: регулярное его превращает `re.escape` ниже. Прежние `\-` и `\]`
#: управляющими последовательностями не являются, поэтому Python оставлял
#: обратную косую в наборе как обычный символ и ругался предупреждением
#: (4.5 ТЗ). Разделителем между номером и названием она никогда не была.
SEPARATOR_CHARS = " \t.:_-–—―)]},;、，。：；·・|«»\"'"
...
#: Номер главы в ЗАГОЛОВКЕ, пришедшем из книги. Номер обязан стоять в
#: начале — самое большее за словом, которое и означает «глава».
#:
#: Впереди допускается любая небуквенная мелочь: кавычка, звёздочка,
#: скобка. Слово перед номером — только из списка: «Глава», «Chapter»,
#: «第». Любое другое слово номер отменяет.
TITLE_NUMBER = re.compile(
    rf"^[^\w\n]*(?:{CHAPTER_WORD}|第)?[\s.№#\-–—]*(?<!\d)(\d{{1,5}})(?!\d)",
    re.IGNORECASE)


def parse_title(text: str) -> NameParts:
    """Номер, часть и название — из заголовка главы внутри книги.

    Отличие от `parse` — в строгости, и оно принципиально.

    Имя файла человек даёт сам, и первая группа цифр в нём почти всегда и
    есть номер главы: «151 Начало» — сто пятьдесят первая. Поэтому разбор
    имён ищет цифры где угодно, и это правильно.

    Заголовок внутри книги пишет чужая программа, и цифра в нём чаще
    всего — часть названия. Из епаба на четыреста сорок шесть глав
    прежний разбор доставал «Act 3 is about to begin.» и объявлял её
    третьей главой; книга потом сортировалась по этим числам и
    перемешивалась вся.

    Поэтому здесь номер обязан стоять в начале — самое большее за словом
    «Глава», «Chapter» или «第». Нет такого — номера нет вовсе, и это
    честнее выдуманного: порядок глав в книге и так известен.
    """
    text = str(text or "").strip()
    found = TITLE_NUMBER.match(text)
    if not found:
        return NameParts(title=text)
    ...
    if piece:
        part = int(piece.group(1))
        rest = rest[piece.end():]
    return NameParts(number=int(found.group(1)), part=part, title=_title(rest))
