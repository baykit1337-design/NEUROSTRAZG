...
#: На сколько правок вглубь искать похожее написание.
#:
#: Здесь была причина, по которой проверка «зависала и ничего не делала».
#: По умолчанию `pyspellchecker` ищет на две правки, а это перебор всех
#: слов, отличающихся двумя буквами: на длинном слове — до полусекунды.
#: Находок в книге бывает пятьсот, и на них уходили минуты — молча, уже
#: после того как главы прочитаны и полоса дошла до конца.
#:
#: Замерено на настоящих опечатках: на две правки словарь угадывает семь
#: из десяти, на одну — шесть, и делает это мгновенно. Одна лишняя
#: догадка не стоит минут ожидания, тем более что почти все находки в
#: переводной книге — имена, которым подсказка не нужна вовсе: их вносят
#: в свой словарь кнопкой «это имя».
EDITS = 1
...
class _Dictionary:
    ...
    def __init__(self, extra=None):
        ...
        try:
            ...
            self._speller = SpellChecker(language="ru", distance=EDITS)
...
def check(targets, folder="", use_registry: bool = True,
    ...
    #
    # О ходе говорим вслух и отмену слушаем. Раньше этот кусок молчал, и
    # выглядело это так: полоса дошла до последней главы и встала. Сколько
    # осталось — не сказано, остановить — нельзя.
    shown = report.findings[:LIMIT]
    for at, finding in enumerate(shown, 1):
        if progress:
            progress.check()
            progress.step(at, len(shown),
                          f"Подбираем замены… {at} из {len(shown)}")
