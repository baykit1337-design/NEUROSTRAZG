...
KINDS = {
    ...
    "token": "Случайный набор букв и цифр",
    "link": "Голая ссылка",
    "cjk": "Иероглифы",
    "latin": "Не переведено",
...
#: Что отмечается сразу, без разбора.
#:
#: Заголовок и название книги — настоящая беда загрузчика: второй
#: заголовок он прочитает как начало новой главы, а название уедет в
#: текст. Случайный набор знаков и голая ссылка — беда другого рода:
#: они не текст книги вовсе, и оставлять их не за чем. Всё остальное —
#: на выбор: непереведённый абзац человек, может, и хочет перевести, а
#: не выбросить.
SPOILS = frozenset({"echo", "repeat", "token", "link"})
...
class Report:
    ...
    def summary(self) -> str:
        ...
        if spoils:
            # Не «мешает загрузчику»: сюда же попал мусор, который
            # загрузчику не мешает вовсе, а книге не нужен — ключи из
            # служебных полей сайта и голые ссылки.
            parts.append(f"лишнее наверняка: {spoils}")
        if rest:
...
#: Ссылка — целой строкой. Внутри фразы ссылку не трогаем: там она
#: часть текста, а вот строка, состоящая из одного адреса, в книге
#: взялась не от автора.
#:
#: Концы строки здесь — для чтения: проверяется каждое слово отдельно, а
#: в слове пробелов нет, и `\S+` и без них дошло бы до конца.
LINK = re.compile(r"^https?://\S+$", re.IGNORECASE)

#: Из чего состоит случайный набор знаков. Ровно алфавит base64 — им
#: пишут ключи, подписи и всё, что попадает в текст из служебных полей
#: сайта.
TOKEN_CHARS = re.compile(r"^[A-Za-z0-9+/=_-]+$")

#: Короче этого случайным набором не считаем.
#:
#: Человек просил ловить «три-пять английских букв подряд». Так нельзя:
#: три латинские буквы — это «SSS», «OK», имя героя и половина названий,
#: и книга осталась бы без них. А настоящий мусор всегда длинный: это
#: закодированная строка, и коротким он не бывает.
TOKEN_MIN = 16


def _random_word(word: str) -> bool:
    """Похоже ли слово на случайный набор знаков, а не на слово.

    Три приметы разом: длина, алфавит base64 и то, что в нём есть и
    цифры, и большие буквы, и малые. Каждая по отдельности встречается в
    обычном тексте — все три вместе не встречаются: длинных слов из
    смеси регистра с цифрами человек не пишет.
    """
    word = word.strip(".,;:!?()[]{}«»\"'…")
    if len(word) < TOKEN_MIN or not TOKEN_CHARS.match(word):
        return False
    return (any(one.isdigit() for one in word)
            and any(one.isupper() for one in word)
            and any(one.islower() for one in word))
...
def _artefact(body: str) -> str:
    ...
    # Мусор из служебных полей сайта — раньше проверки на кириллицу и
    # раньше «не переведено»: и ключ, и ссылка состоят из латиницы, и
    # без этого они прятались бы среди настоящего английского текста,
    # который человек, может, хочет перевести, а не выбросить.
    words = body.split()
    if all(LINK.match(one) for one in words):
        return "link"
    if all(_random_word(one) for one in words):
        return "token"
