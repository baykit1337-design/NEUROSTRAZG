...
#: Заглушка зонда — та самая, что приходит вместо страницы.
#:
#: Не догадка и не образец из чужого проекта: снята с живого ответа
#: Цидяня, все 209 байт целиком —
#:
...
#:
#: Тела нет вовсе, есть только зонд. Признаком берём `buid` из девятнадцати
#: букв «f» — это «читатель не опознан»; сам адрес зонда в признаки не
#: годится, он висит в шапке и совершенно рабочей страницы.
PROBE = 'buid = "fffffffffffffffffff"'
...
def _diagnose(page: str, address: str) -> str:
    ...
    if PROBE in said:
        # Отдельно от общей проверки на робота: эта заглушка узнаётся
        # дословно, и сказать про неё можно точно, а не «похоже на».
        return (f"Цидянь отдал не рейтинг, а заглушку своего зонда "
                f"({address}): пустое тело и один скрипт `probe.js`. Так его "
                f"защита встречает всякого, чей пропуск (куку `w_tsfp`) она "
                f"не приняла, — а мы отправили обе известные формы подписи. "
                f"Значит, правило подписи изменилось или его мало: заглушка "
                f"приходит одинаково и через прокси, и напрямую, так что "
                f"дело не в адресе выхода. Остаётся брать страницу настоящим "
                f"браузером. Страница {len(said)} байт{tail}.")
