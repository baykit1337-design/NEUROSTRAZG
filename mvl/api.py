...
class Chapter:
    ...
    ch_name: str = ""
    link: str = ""
    #: Глава закрыта, и это видно уже по оглавлению.
    #:
    #: Источнику, который узнаёт про замок только на самой главе, поле
    #: не нужно — оно и остаётся ложью по умолчанию. А там, где сайт
    #: помечает закрытые главы в списке, это экономит прогон целиком:
    #: у книги на тысячу двести глав открыто бывает десять, и без метки
    #: качалка честно сходит за каждой из тысячи двухсот, чтобы тысячу
    #: сто девяносто раз получить отказ.
    locked: bool = False
...
def _soup(html: str):
    ...
    except Exception as exc:  # noqa: BLE001 — lxml необязателен
