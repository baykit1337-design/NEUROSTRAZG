 ...
 *
 * Движения медленные, но не незаметные. Первая версия была настолько
 * осторожной, что поле не отличалось от картинки: мерцание в полтора
 * десятка сотых яркости и один переезд в несколько секунд глаз не ловит.
 * Теперь точки заметно дышат и по одной перебираются на новые места —
 * фон живой, но по экрану по-прежнему ничто не носится.
 */
...
//:
//: Размах яркости широкий намеренно. С прежним (0.10→0.25 у дальних) поле
//: мерцало по приборам, но не для глаза: на тёмном фоне разница в полтора
//: десятка сотых незаметна, и небо выглядело нарисованным.
const STARS_DEPTHS = [
  {share: 0.60, dim: 0.04, bright: 0.42, size: 1.0, glow: false},
  {share: 0.30, dim: 0.10, bright: 0.70, size: 1.5, glow: false},
  {share: 0.10, dim: 0.20, bright: 1.00, size: 2.2, glow: true},
];

//: Мерцание: длительность полного цикла у каждой точки своя. Короче, чем
//: было: на цикле в двенадцать секунд точка меняется медленнее, чем
//: человек успевает заметить, и поле кажется застывшим.
const STARS_BLINK_MIN = 2200;
const STARS_BLINK_MAX = 6500;

//: Переезд: раз в столько-то одна точка гаснет и появляется в новом месте.
const STARS_MOVE_MIN = 900;
const STARS_MOVE_MAX = 2200;

//: Сколько длится угасание и сколько — появление.
const STARS_FADE_MIN = 1200;
const STARS_FADE_MAX = 2600;

//: Сколько точек могут переезжать одновременно. Одна на всё поле — это
//: одно событие в несколько секунд на полторы сотни точек: не заметить.
const STARS_MOVING_MAX = 12;
