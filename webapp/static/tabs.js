...
/** Версия в шапке.
 *
 * Спрашивается у сервера, а не вписана в страницу: страница кэшируется
 * браузером, и после обновления в шапке ещё сутки висела бы прошлая
 * версия. Вопрос «что у меня стоит» задают как раз тогда, когда что-то
 * пошло не так, и неверный ответ на него дороже всего.
 *
 * Не ответил — оставляем пусто: пустая метка спрятана стилями, и это
 * лучше, чем «версия неизвестна» посреди шапки.
 */
async function showVersion(){
  const box = $('appVersion');
  if(!box) return;
  try{
    const data = await call('/api/about');
    if(data.version) box.textContent = 'v' + data.version;
  }catch(err){ /* без версии интерфейс работает так же */ }
}
